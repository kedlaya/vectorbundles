...
lemma quadratic_algebraic_closure (h : Module.finrank F K = 2) :
  ...
  have h_gdeg: g.natDegree = 4 ∧ g ≠ 0 ∧ g.degree = 4 := by
    have h1 : g.natDegree ≤ 4 :=
      natDegree_le_iff_coeff_eq_zero.mpr hg5
    ...
    have h0 : g ≠ 0 := by
      ...
      simp_all
    refine ⟨hdeg, h0, ?_⟩
    have : g.degree = g.natDegree := degree_eq_natDegree h0
    simp_all
  have h_gmon : g.Monic := by
    refine monic_of_natDegree_le_of_coeff_eq_one 4 ?_ ?_
    · aesop
    · simp_all
  have hf : ∃ f0 : Polynomial F, f0.Monic ∧ f0.natDegree = 2 ∧ f0 ∣ g := by
    have : IsAlgClosed K := IsAlgClosure.isAlgClosed F
    have hc : ∃ c : K, aeval c g = 0 := by
    ...
    have h_fdeg : f.natDegree = 1 ∨ f.natDegree = 2 := by
      ...
      have : f.natDegree ≤ g.natDegree := by
        refine natDegree_le_of_dvd h_div ?_
        exact Monic.ne_zero h_gmon
      have : (minpoly F c).natDegree ≤ Module.finrank F K := minpoly.natDegree_le c
      grind
    cases h_fdeg
    · have h_e : ∃ (e : Polynomial F), e * f = g ∧ e.Monic ∧ e.natDegree + f.natDegree = g.natDegree
      ...
      have : e.natDegree = 3 := by grind
      have h_div1a : e ∣ g := dvd_of_mul_right_eq f h_ef1
      have hc : ∃ c : K, aeval c e = 0 := by
      ...
      have h_fdeg : f.natDegree = 1 ∨ f.natDegree = 2 := by
        have : f.natDegree > 0 := minpoly.natDegree_pos h_int
        have : f.natDegree ≤ e.natDegree := by
          refine natDegree_le_of_dvd h_div2 ?_
          exact Monic.ne_zero h_emonic
        have : (minpoly F c).natDegree ≤ Module.finrank F K := minpoly.natDegree_le c
        grind
  ...
  have : (f.coeff 0) + (e.coeff 1) * (f.coeff 1) + (e.coeff 0) = -2*a := by
    specialize h_prod 2
    rw [Finset.Nat.sum_antidiagonal_eq_sum_range_succ_mk] at h_prod
    unfold Finset.range at h_prod
    simp_all only [gt_iff_lt, Multiset.range_succ, Multiset.range_zero, Multiset.cons_zero, Finset.mk_cons,
      Finset.cons_eq_insert, Finset.mem_insert, OfNat.ofNat_ne_one, Finset.mem_mk,
      Multiset.mem_singleton, OfNat.ofNat_ne_zero, or_self, not_false_eq_true, Finset.sum_insert,
      one_ne_zero, Finset.sum_mk, Multiset.map_singleton,
