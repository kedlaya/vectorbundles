...
lemma divisor_by_finrank (f : F[X]) {p : ℕ} (hr : finrank F K ∣ p) (hp : p.Prime) :
  (∃ x, f.IsRoot x) ∨ (∀ d, Irreducible d → d ∣ f → d.natDegree = p) := by
  by_cases h : ∃ x, f.IsRoot x; left; exact h; /- -/ right; intro d hd1 hd2
  rcases (dvd_prime hp).mp ((finrank_divides F K hd1).trans hr) with h | h
...
lemma divisor_by_finrank' (f : F[X]) {p : ℕ} (hr : finrank F K ∣ p) (hp : p.Prime):
  ...
  rw [hd, natDegree_multiset_prod S (prod_eq_zero.mt h2)]; apply dvd_sum; simp only [mem_map]
  rintro - ⟨d, hd, rfl⟩; exact (h1 d (irreducible_of_factor d hd) (dvd_of_mem_factors hd)).symm.dvd
...
lemma finite_alg_closure_prime {p : ℕ} [IsGalois F K] (hp : p.Prime) (hr : finrank F K = p) :
    ¬CharP F p := by open CanLift Field FiniteDimensional IsAlgClosed minpoly Subfield in
  by_contra; open Prime in have hp0 := pos hp; have hp1 := one_lt hp
  ...
  have h := (coe_lt_degree.mpr ((h_a ((algebraMap F K) a * x^(p-1))).trans_gt hp0)).ne'
  have := artin_schreier_tower F K hp a x ha; subst hr; have := of_finrank_pos hp0
  have ⟨y, hy⟩ := exists_aeval_eq_zero K _ h; have : y ∈ F⟮x⟯ := by
    simp [(primitive_element_iff_minpoly_natDegree_eq F x).mpr h_d]
  simp_all only [map_pow, Subtype.exists, aeval_sub, coe_aeval_eq_eval, aeval_X, eval_C]; grind
...
lemma finite_insep_alg_closure [FiniteDimensional F K] [IsPurelyInseparable F K] : PerfectField F
  := by open FaithfulSMul Function Injective IsAlgClosed IsPurelyInseparable in
  let p := ringChar F; let n := exponent F K; rcases CharP.char_is_prime_or_zero F p with h | h
  · have := ExpChar.prime h (R := F); have h : Surjective (frobenius F p) := by
      intro b; have ⟨x, hx⟩ := exists_pow_nat_eq ((algebraMap F K) b) (expChar_pow_pos F p (n + 1))
      use elemReduct F x ^ p ^ (n - elemExponent F x); apply (eq_iff (algebraMap_injective F K)).mp
      rw [←hx, Nat.pow_succ', ←pow_mul_pow_sub p (elemExponent_le_exponent F x), frobenius_def,
        ←pow_mul, map_pow, algebraMap_elemReduct_eq' F p]; ring
    open PerfectRing in have := ofSurjective F p h; exact toPerfectField F p
  · have := (CharP.ringChar_zero_iff_CharZero F).mp h; exact PerfectField.ofCharZero
...
lemma quadratic_alg_closure (h : finrank F K ∣ 2) (a b : F) : IsSquare (a*a+b) ∨ IsSquare (-b)
    := by open And Finset IsMonicOfDegree in
  ...
  let h2 := prime_two; rcases divisor_by_finrank F K g h h2 with ⟨x, hx⟩ | h
  · right; use x^2 - a; simp only [IsRoot.def, g, eval_add, eval_monomial] at hx; grind only
  · have h0 := exists_monic_irreducible_factor g (not_isUnit_of_natDegree_pos g (by grind [h1.1]))
    obtain ⟨f, h3, h4, ⟨e, he⟩⟩ := h0; have hf : f.IsMonicOfDegree 2 := ⟨h f h4 ⟨e, he⟩, h3⟩
    ...
    have : f₀*e₀ = a^2 + b ∧ f₀*e₁ + f₁*e₀ = 0 ∧ f₀ + f₁*e₁ + e₀ = -2*a ∧ f₁ + e₁ = 0 := by
      ...
      have hm' := intro (hm 0) (intro (hm 1) (intro (hm 2) (hm 3))); rw [he] at h1
      simp [antidiagonal, h hf, h (of_mul_left hf h1)] at hm'; ring_nf at hm' ⊢; exact hm'
...
lemma finite_alg_closure_i [FiniteDimensional F K] (hm : IsSquare (-1 : F)) : IsAlgClosed F := by
  ...
  have : IsAlgClosure F K := ⟨Hac, inferInstance⟩; have h : finrank F K = 1 := by
    by_contra h; have := isGalois_iff.mpr ⟨inferInstance, IsAlgClosure.normal F K⟩
    rw [←IsGaloisGroup.card_eq_finrank Gal(K/F)] at h; have ⟨p, hp, hp2⟩ := exists_prime_and_dvd h
    have := fact_iff.mpr hp; have hp0 := Prime.pos hp; have hp1 := Prime.one_lt hp
    have ⟨g, hg⟩ := exists_prime_orderOf_dvd_card' p hp2; let H := Subgroup.zpowers g
    have hr := finrank_fixedField_eq_card H; rw [card_zpowers, hg] at hr; let E := fixedField H
    have h_char := finite_alg_closure_prime E K hp hr; have hG := IsGalois.card_aut_eq_finrank E K
    have hK : (primitiveRoots p E).Nonempty := by
      rcases divisor_by_finrank' E K _ hr.dvd hp with ⟨z, hz⟩ | hz
      · have : NeZero (p : E) := ⟨(CharP.charP_iff_prime_eq_zero hp).mpr.mt h_char⟩
        exact ⟨z, (mem_primitiveRoots hp0).mpr (isRoot_cyclotomic_iff.mp hz)⟩
      · exfalso; rw [natDegree_cyclotomic p E, totient_prime hp] at hz
        grind only [le_of_dvd (zero_lt_sub_of_lt hp1) hz]
    ...
    have ⟨a, h2, _⟩ := h hK ⟨inferInstance, isCyclic_of_prime_card hG⟩; have hp : p = 2 := by
      ...
      have h := finrank_divides E K h; rw [hr] at h; simp_all [not_pos_pow_dvd hp1 one_lt_two]
    rw [hp] at h2 hr; let h := quadratic_alg_closure E K hr.dvd 0 a; ring_nf at h
    have ha := nthRoots_two_eq_zero_iff.mp (roots_eq_zero_of_irreducible_of_natDegree_ne_one h2 ?_)
    open IsSquare in have := mul (map (algebraMap F E) hm) (Or.resolve_left h ha); repeat aesop
...
omit Hac in lemma RealClosed_from_quadratic (h1 : ¬IsSquare (-1 : F))
  ...
  have hi : IsIntegral F i := by apply of_pow two_pos; rw [h2a]; exact neg isIntegral_one
  ...
  have hq := quadratic_alg_closure F F⟮i⟯ hr; have hssq : ∀ x : F, IsSumSq x → IsSquare x := by
    apply IsSumSq.rec'; simp; rintro a b ⟨y, rfl⟩ - h; by_cases 0 = b; · use y; grind only
    · by_contra; have h1 : IsSquare (-b) := (by grind); grind only [IsSquare.div h1 h]
  have := isSemireal_iff_not_isSumSq_neg_one.mpr ((hssq (-1)).mt h1)
...
theorem artin_schreier_thm [FiniteDimensional F K] : IsAlgClosed F ∨ IsRealClosed F := by
  ...
  right; have ⟨i, hi⟩ := IsAlgClosed.exists_pow_nat_eq (-1 : K) two_pos
  have h := finite_alg_closure_i F⟮i⟯ K ((isSquare_iff_exists_sq (-1)).mpr ?_)
  exact RealClosed_from_quadratic F K hF ⟨i, ⟨hi, h⟩⟩; symm at hi; aesop
