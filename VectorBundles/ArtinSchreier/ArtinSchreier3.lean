        ...
        have _ : finrank F K ≤ 1 := by
          ...
          have h_E1 : finrank E K = 1 := by
            calc
              ...
              _ = 1 := by simp
          grind
        have h_rank1 : finrank F K = 1 := by
          (expose_names; refine Eq.symm (Nat.le_antisymm ?_ h_2))
          refine Nat.one_le_iff_ne_zero.mpr ?_
          refine Nat.ne_zero_iff_zero_lt.mpr ?_
          apply finrank_pos
        have _ : Field.finSepDegree F K * Field.finInsepDegree F K = finrank F K :=
          Field.finSepDegree_mul_finInsepDegree F K
        have h_insep1 : Field.finInsepDegree F K = 1 := by
          have h : Field.finInsepDegree F K ∣ finrank F K := by
            (expose_names; exact dvd_of_mul_left_eq (Field.finSepDegree F K) h_3)
          rw [h_rank1] at h
          aesop
