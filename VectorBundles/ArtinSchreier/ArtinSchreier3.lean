...
lemma finite_algebraic_closure_cyclic_quadratic (F : Type) (K : Type) {p : ℕ} [Field F] [Field K]
    ...
    (hrank : Module.finrank F K = p) : p = 2 ∧ (∃ a : F, ¬ IsSquare a) ∧
    (∀ a : F, ¬ IsSquare a → IsSquare (-a)) := by open Nat Polynomial in
  ...
  have hp2 : p = 2 := by
    ...
    have := (h 2 (by simp)).mpr ((h 1 one_ne_zero).mp this)
  ...
  rw [hp2] at h2 hrank; refine ⟨hp2, ⟨a, nthRoots_two_eq_zero_iff.mp ?_⟩, ?_⟩
  · apply roots_eq_zero_of_irreducible_of_natDegree_ne_one h2; aesop
  · intro a ha; cases (quadratic_algebraic_closure F K hrank 0 a) with
