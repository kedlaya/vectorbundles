...
With notation as in Lemma~\ref{lem:filtration by O_m}, we can further ensure that 
\[
...
Set $m := m_i+1 = m_j-1$;
for $k=i+1,\dots,j-1$, the induction hypothesis yields $m = m_i + 1 \leq m_k \leq m_j=1-m$,
so $(m_i, \dots, m_j) = (m-1, m, \dots, m, m+1)$.
That is, $(F_j/F_i)(-m)$ admits a filtration with successive quotients $\cO_X(-1), G, \cO_X(1)$
...
\begin{lemma} \label{lem:sequence argument}
...
\end{proposition}
\begin{proof}
\uses{lem:minimal filtration,lem:sequence argument,lem:H1 of line bundles}
Let $F$ be a semistable vector bundle of degree $0$ and rank $n$,
and choose a filtration as in Lemma~\ref{lem:minimal filtration}.
By Lemma~\ref{lem:sequence argument}, we have $m_1 = \cdots = m_n = 0$.
