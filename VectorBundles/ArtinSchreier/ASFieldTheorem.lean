...
lemma finite_algebraic_closure_cyclic_prime {p : ℕ} [IsGalois F K] (hp : Nat.Prime p)
  ...
  by_contra; open CanLift FiniteDimensional IsAlgClosed minpoly Nat Subfield in
  have hp0 := Prime.pos hp; have hp1 := Prime.one_lt hp
...
lemma finite_inseparable_algebraic_closure [FiniteDimensional F K] [IsPurelyInseparable F K] :
    PerfectField F := by open CharP FaithfulSMul Function IsAlgClosed IsPurelyInseparable in
  let p := ringChar F; let n := exponent F K; rcases char_is_prime_or_zero F p with hp | hp
...
lemma finite_algebraic_closure_with_i [FiniteDimensional F K] (hm : IsSquare (-1 : F)) :
    IsAlgClosed F := by open IntermediateField IsAlgClosed IsGaloisGroup IsSquare Nat in
  ...
  have : IsAlgClosure F K := ⟨Hac, inferInstance⟩; have h : finrank F K = 1 := by
    ...
    rw [←card_eq_finrank Gal(K/F), card_eq_fintype_card]; by_contra h1
    have ⟨p, hp, hp2⟩ := exists_prime_and_dvd h1; have := fact_iff.mpr hp; have hp0 := Prime.pos hp
    have hp1 := Prime.one_lt hp; have ⟨g, hg⟩ := exists_prime_orderOf_dvd_card p hp2
    let H := Subgroup.zpowers g; let E := fixedField H; have hr := finrank_fixedField_eq_card H
    rw [card_zpowers, hg] at hr; have hG := IsGalois.card_aut_eq_finrank E K
    have hK : (primitiveRoots p E).Nonempty := by
      rcases divisor_by_finrank' E K _ hr.dvd hp with ⟨z, hz⟩ | hz
      · have h_char := finite_algebraic_closure_cyclic_prime E K hp hr
      ...
      · rw [natDegree_cyclotomic p E, totient_prime hp] at hz
    ...
    have ⟨a, h2, _⟩ := h hK ⟨inferInstance, isCyclic_of_prime_card hG⟩; have hp : p = 2 := by
      by_contra; have h := fun n ↦ X_pow_sub_C_irreducible_iff_of_prime_pow hp this (K := E)
    ...
    have := mul (map (algebraMap F E) hm) (Or.resolve_left h ha); repeat aesop
  have h := bot_eq_top_iff_finrank_eq_one.mpr h
  exact of_ringEquiv K F (topEquiv.symm.trans ((equivOfEq h).symm.trans (botEquiv F K)))
...
omit Hac in lemma RealClosed_from_quadratic (h1 : ¬IsSquare (-1 : F))
  ...
  obtain ⟨i, h2a, _⟩ := h2; open adjoin IsIntegral minpoly Nat in /- -/
  have hr : finrank F F⟮i⟯ ∣ 2 := by
    have hi : IsIntegral F i := by apply of_pow two_pos; rw [h2a]; exact neg isIntegral_one
    have h := natDegree_le_of_dvd (dvd_iff (x := i).mpr ?_) (X_pow_add_C_ne_zero two_pos (1 : F))
    rw [finrank hi]; apply (dvd_prime prime_two).mpr; rw [natDegree_X_pow_add_C] at h
    grind only [natDegree_pos hi]; aesop
  ...
  have hq := quadratic_alg_closure F F⟮i⟯ hr; have hssq : ∀ x : F, IsSumSq x → IsSquare x := by
    apply IsSumSq.rec'; simp; rintro a b ⟨y, rfl⟩ _ _; by_cases h : 0 = b; use y; repeat grind
...
/- The Artin-Schreier theorem: a field admitting an algebraic closure which is a finite extension
