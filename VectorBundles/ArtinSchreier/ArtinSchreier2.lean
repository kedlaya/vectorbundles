  ...
  have h_matchcoeff : y1p_rep.coeff (p-1) = c^p := by
    ...
    have h := by
      ...
      have h_eval := calc
        ...
        _ = aeval (x + iota a) m := by rw [aeval_add, aeval_X, aeval_C]
        _ = aeval (aeval x ((X : Polynomial F) ^ p)) m := by rw [←h_rwx, aeval_X_pow]
        _ = aeval x (expand F p m) := (aeval_comp x).symm
        _ = aeval x (y_rep ^ p) := by rw [←map_expand, map_frobenius_expand]
        _ = ↑(y ^ p) := by rw [map_pow, h7, ←IntermediateField.coe_pow]
      have : aeval x (yp_rep - y1p_rep) = 0 := by
        rw [aeval_sub, aeval_add, h_eval, hy4, h7, aeval_monomial]
        aesop
      have hdiv := minpoly.dvd_iff.mpr this
      have : (yp_rep - y1p_rep).natDegree < p := by
        grind [natDegree_add_le, natDegree_sub_le, natDegree_monomial_le]
      rw [←h_natdeg] at this
      exact sub_eq_zero.mp (eq_zero_of_dvd_of_natDegree_lt hdiv this)
