...
lemma cyclic_char_p_as_artin_schreier (F K : Type) [Field F] [Field K] [Algebra F K]
  ...
  have := Prime.pos hp; rw [←hrank] at this
  have := FiniteDimensional.of_finrank_pos this
  have := (Algebra.charP_iff F K p).mp ‹CharP F p›
  let G := Gal(K/F); have h_ord := IsGalois.card_aut_eq_finrank F K; rw [hrank] at h_ord
  ...
  have hz2 : ∑ i : rp, (g^(i+1:ℕ)) y = ∑ σ : G, σ y := by open IsOfFinOrder in
    ...
    · have : DecidableEq G := Classical.typeDecidableEq G
      ...
      rw [h_ordg, mem_image] at this; have ⟨i, hb1, hb2⟩ := this
  ...
  have hz3 := calc
    ∑ σ : G, σ y = (algebraMap F K) (trace F K y) := (trace_eq_sum_automorphisms y).symm
  ...
  have := calc
    ...
    _ = z - 1 := by rw [hz1, hz2, hz3]
  let b := z^p - z
  have : b ∈ (⊥ : IntermediateField F K) := by
    apply (IsGalois.mem_bot_iff_fixed b).mpr; intro h
    ...
    refine mem_fixedBy_zpowers_iff_mem_fixedBy.mpr (mem_fixedBy.mpr ?_) n
    calc
      g b = g (z^p) - g z := map_sub g (z ^ p) z
      _ = b := by simp [map_pow, this, sub_pow_char, sub_sub_sub_cancel_right, b]
  ...
  have h : (X ^ p - X - C a).natDegree ≤ (minpoly F z).natDegree := by open minpoly in
    ...
    by_contra
    have := (IsGalois.mem_range_algebraMap_iff_fixed z).mp (natDegree_eq_one_iff.mp (by aesop)) g
    grind only
