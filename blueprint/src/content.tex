...
Let $S$ be a nonempty subset of $\Z^n$ satisfying the following conditions.
\begin{enumerate}
\item[(a)]
For $i=1,\dots,n-1$, for $(m_1,\dots,m_n) \in S$, the values of $m_1+\cdots+m_i$ are uniformly bounded above.
\item[(b)]
For $i=1,\dots,n$, for $(m_1,\dots,m_n) \in S$, the values of $m_1+\cdots+m_n$ are all equal.
\item[(c)]
If $(m_1,\dots,m_n) \in S$ and there exists an index $i \in \{1,\dots,n-1\}$ such that 
$m_{i+1} > m_{i}+1$, then there exists $(m'_1,\dots,m'_n) \in S$
with 
\[
m'_i > m_i, \qquad m_k = m_k \quad (k \notin \{i,i+1\}).
\]
\item[(d)]
If $(m_1,\dots,m_n) \in S$ and there exist indices $i,j$ with $1 \leq i < j \leq n$ such that 
$(m_i,\dots,m_j) = (m-1, m,\dots,m,m+1)$ for some $m \in \Z$,
then there exists $(m'_1,\dots,m'_n) \in S$ with
\[
m'_i = \cdots = m'_j = m, \qquad m_k = m_k \quad (k \notin \{i,\dots,j\}).
\]
\end{enumerate}
Then there exists $(m_1,\dots,m_n) \in S$ for which no indices exist as in (c) or (d).
\end{lemma}
\begin{proof}
By (a) and (b) and the nonemptiness of $S$, there exists $(m_1,\dots,m_n)$ which minimizes
\[
\sum_{i=1}^n i m_i 
= (n+1) \deg(F) -\sum_{i=1}^{n-1} (m_1 + \cdots + m_i).
\]
As this value is increased by any substitution as in (c) or (d), no such substitution is possible.
\end{proof}
