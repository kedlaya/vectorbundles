...
lemma quadratic_algebraic_closure (h : finrank F K = 2) (a b : F) : IsSquare (a*a+b) ∨ IsSquare (-b) := by
  ...
  obtain ⟨f, hf1, hf2, hf3⟩ : ∃ f0, f0.Monic ∧ f0.natDegree = 2 ∧ f0 ∣ g := by
    have : IsAlgClosed K := IsAlgClosure.isAlgClosed F
    have h_divh : ∀ h : Polynomial F, h.natDegree ≠ 0 → ∃ f,
    ...
    · obtain ⟨e, h_ef1, h_emonic, _⟩ := polynomial_monic_divisor h_fmon h_gmon h_div
      ...
      obtain ⟨f1, h_fmon, h_fdeg, h_div⟩ := h_divh e he0
      cases h_fdeg with
      | inl =>
        obtain ⟨d, h_d1, h_dmonic, _⟩ := polynomial_monic_divisor h_fmon h_emonic h_div
        use d
        have h_div4 := dvd_of_mul_right_eq f1 h_d1
