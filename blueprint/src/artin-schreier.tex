...
If $a \in F$ is not a $p$-th power, then
for every nonnegative integer $n$, the polynomial $x^{p^n}-a$ is irreducible over $F$.
\end{lemma}
\begin{proof}
Let $K$ be an algebraic closure of $F$.
Let $b \in K$ be an element with $b^{p^n} = a$.
Then $x^{p^n}-a$ factors over $K$ as $(x-b)^{p^n}$,
so any irreducible factor over $F$ itself factors over $K$ as $(x-b)^i$ for some $i \in \{1,\dots,p^n\}$. This implies $b^i \in F$, but since $a$ is not a $p$-th power in $F$ this is only possible if $i \equiv 0 \pmod{p^n}$.
\end{proof}
...
\lean{Field.finite_algebraic_closure_cyclic_quadratic}
...
By Kummer theory, $K  \cong F[x]/(x^p-a)$ for some $a \in F$. However, Lemma~\ref{lem:p-power extensions}
now implies that $x^{p^n}-a$ is irreducible over $F$ for each positive integer $n$, a contradiction.
\end{proof}
...
\lean{Field.finite_algebraic_closure_with_i}
...
Let $K$ be an algebraic closure of $F$.
Let $i \in K$ be a square root of $-1$.
By Lemma~\ref{lem:ac-finite-alg-closure-with-i}, $F(i) = K$, so either $i \in F$
and $F$ is algebraically closed, or $i \notin F$ and 
$F$ is real closed by Lemma~\ref{lem:real-closed-from-quadratic}, $F$ is real closed.
\end{proof}
